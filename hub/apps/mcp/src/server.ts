...
- get_heatmap shows where attention went: most-clicked and most-hovered elements, key sections seen, dead clicks (people click something that does nothing) and rage clicks, and the scroll drop-off. Compare variants and devices (a hero that works on desktop can bury the button on mobile). Turn it into concrete next tests with selectors.
- list_library holds every past result across all sites. Before proposing a test, check it for something that already won elsewhere; if you reuse one, say where it came from and that a different audience may behave differently. apply_library_item drafts it (a draft test with the change loaded, or a draft post); apply_library_change_permanently goes live and needs the same care as apply_winner.
